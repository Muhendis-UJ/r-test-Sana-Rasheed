...
```{r }
    bank_customers <- data.frame(CustomerID=c("Alicia","Nancy",
                                              "Fernando","Louis","Diane"), 
                                 hasAccount=c("No","Yes",
                                              "Yes","Yes","NA"), 
                                 totalBalance=c("NA","usd 10,000",
                                              "usd 100","usd 2,000","NA"))
```
...
### Question 2
**Create a matrx of size 3x3 called mat_1:**  
...
```{r }
    ...
    for(i in 1:3){
        for(j in 1:3){
            print(sprintf("mat_1 = %i, nrow = %i, ncol = %i",mat_1[i,j], i, j))
        }
    }   
```
...
```{r }
    ...
    gcd <- function(a, b) {
        ...
        print(sprintf("GCD of %i & %i is = %i", a1, a2, a))
        return(a)
    }
    answer <- gcd(75,125)
```
...
```{r }
    ...
    #matrix multiplication cannot perform because different dimension
    matrix_1 %*% matrix_2
```
...
```{r }
    ...
    b <- as.POSIXct("2020-6-27 21:00:00",tz = "Etc/GMT-5")
    a-b
```
...
```{r }
    ...
    rmeans = rowMeans(mymat)
    cmeans = colMeans(mymat)
    paste(c("Row-wise mean = ",rmeans),collapse=" ")
    paste(c("Col-wise mean = ",cmeans),collapse=" ")
```
